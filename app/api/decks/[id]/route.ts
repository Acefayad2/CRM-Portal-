...
export async function GET(
  ...
  try {
    ...
    const { data: deck, error: deckError } = await actor.client
      .from("slide_decks")
      .select("id, title, created_at, owner_user_id")
      .eq("id", id)
      .single()

    if (deckError || !deck || deck.owner_user_id !== actor.userId) {
      return NextResponse.json({ error: "Deck not found" }, { status: 404 })
    }

    const { data: slides } = await actor.client
      .from("slides")
...
export async function DELETE(
  ...
  try {
    ...
    const { error } = await actor.client.from("slide_decks").delete().eq("id", id)
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
